...
class SuiteAccountsPage(AdminPage, BasePage):
    ...
    def get_accounts_by_suite_holder(self, suite_holder):
        return filter(lambda row: row.SUITE_HOLDER == suite_holder, self.DATATABLE_ROWS)

    def get_accounts_by_phone(self, phone):
        return filter(lambda row: row.PHONE == phone, self.DATATABLE_ROWS)

    def get_accounts_by_suite(self, suite):
        return filter(lambda row: row.SUITE == suite, self.DATATABLE_ROWS)

    def get_account_by_id(self, id):
        for account in self.DATATABLE_ROWS:
            if account.ID == id:
                return account
        else:
            return None
